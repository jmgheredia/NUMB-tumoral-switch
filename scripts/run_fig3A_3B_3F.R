...
#
# Outputs:
# - results/figures/figure3A/Figure_3A_UpSet_isoforms_ordered.tiff
# - results/figures/figure3B/Heatmap_<PATHWAY>_log2FC_isoforms.tiff
# - results/Tables/Supplementary_Table_2_pathways.csv
# - results/figures/figure3F/Figure3F_Metabolic_heatmap_log2FC.tiff
# - results/Tables/Supplementary_Table_4_Figure3F_heatmap_log2FC.csv

# ============================================================
# Figure 3A / 3B / 3F panels
# ============================================================
...
diff_file      <- file.path("results", "processed", "differential_fused_NUMB_isoforms_annotated.csv")
...
fig3A_dir  <- file.path("results", "figures", "figure3A")
fig3B_dir  <- file.path("results", "figures", "figure3B")
fig3F_dir  <- file.path("results", "figures", "figure3F")
tables_dir <- file.path("results", "Tables")

dir.create(fig3A_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(fig3B_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(fig3F_dir, recursive = TRUE, showWarnings = FALSE)
...
rows_to_keep <- apply(upset_input, 1, function(x)
  paste0(which(x == 1), collapse = "_") %in% valid_combos)
...
out_fig_3A <- file.path(fig3A_dir, "Figure_3A_UpSet_isoforms_ordered.tiff")

tiff(out_fig_3A, width = 3000, height = 2000, res = 300, compression = "lzw")
...
if ("gene_symbol" %in% colnames(pathways)) colnames(pathways)[colnames(pathways)=="gene_symbol"] <- "hgnc_symbol"
...
out_table_3B <- file.path(tables_dir,"Supplementary_Table_2_pathways.csv")
...
for (pw in unique(supp_table_3B$pathway)) {
  ...
  out_fig <- file.path(fig3B_dir,paste0("Heatmap_",pw,"_log2FC_isoforms.tiff"))
  
  tiff(out_fig,width=2000,height=1500,res=300,compression="lzw")
...
if ("gene_symbol" %in% colnames(metabolic)) colnames(metabolic)[colnames(metabolic)=="gene_symbol"] <- "hgnc_symbol"

matched <- merge(diff_df, metabolic, by="hgnc_symbol")

selected_pathways <- c("Glycolysis","Pyruvate","TCA_cycle","Oxidative_phosphorylation","Carbon_metabolism")
...
pal <- c(Glycolysis="#E41A1C",Pyruvate="#377EB8",TCA_cycle="#4DAF4A",
         Oxidative_phosphorylation="#984EA3",Carbon_metabolism="#FF7F00")

out_fig_3F <- file.path(fig3F_dir,"Figure3F_Metabolic_heatmap_log2FC.tiff")

tiff(out_fig_3F,width=2800,height=3200,res=400)
...
supp_table_3F <- data.frame(pathway=matched$pathway,
                            hgnc_symbol=matched$hgnc_symbol,
                            transcript_id=matched$transcript_id,
                            mat_met)

out_table_3F <- file.path(tables_dir,"Supplementary_Table_4_Figure3F_heatmap_log2FC.csv")
