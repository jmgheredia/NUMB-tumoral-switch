...
# -----------------------------
# 3) Alternative splicing analysis (pan-cancer)
# -----------------------------
...
# -----------------------------
# 4) Figure 1B: splicing events across cancer types
# -----------------------------
splicing_colors <- c(
  ES = "#CC79A7",
  IR = "#0072B2",
...
# -----------------------------
# 5) BRCA-specific analysis (tumor vs control)
# -----------------------------
...
# -----------------------------
# 6) Figure 1C: BRCA splicing gain vs loss
# -----------------------------
p1C <- ggplot(
  brca_event_counts,
...
# 7.1 Global compositional test (2 x K)
...
# 7.2 Event-level 2x2 tests (proportions)
...
# 7.3 Binomial test (directional bias)
binomial_stats <- brca_event_counts %>%
  dplyr::select(event_type, condition, count) %>%
  pivot_wider(
    names_from = condition,
    values_from = count,
    values_fill = 0
