...
make_dotplot <- function(ego, title, n = 15) {
  df <- as.data.frame(ego)
  df$GeneRatio <- sapply(df$GeneRatio, function(x) eval(parse(text = x)))
  df <- df[order(df$p.adjust), ][1:n, ]
  df <- df[order(df$GeneRatio), ]
  df$Description <- factor(df$Description, levels = df$Description)
  
  ggplot(df, aes(x = GeneRatio, y = Description, size = Count, colour = p.adjust)) +
    geom_point() +
    scale_colour_viridis_c(option = "mako", direction = -1, begin = 0.1, end = 0.9) +
    scale_size_continuous(range = c(3, 10)) +
    scale_x_continuous(expand = expansion(mult = c(0.05, 0.1))) +
    labs(title = title, x = "GeneRatio", y = NULL, colour = "p.adjust") +
    theme_bw() +
    theme(
      plot.margin = margin(5, 5, 5, 20, unit = "mm"),
      panel.grid = element_blank()
    )
}

p_up   <- make_dotplot(ego_up,   "GO enrichment – High NUMB score")
p_down <- make_dotplot(ego_down, "GO enrichment – Low NUMB score")
