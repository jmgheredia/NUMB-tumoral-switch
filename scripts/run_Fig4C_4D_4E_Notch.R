# ============================================================
# run_Figure4C_Notch_PCA_UMAP.R
#
# PCA, UMAP and clustering of Notch pathway genes in PDMR
#
# Inputs:
...
# - results/Figures/Figure_4/Notch_pathway/*.tiff
#
# Intermediate:
# - results/processed/Figure_4/UMAP_coordinates_Notch.csv
# ============================================================
...
fig_dir  <- file.path("results","Figures","Figure_4","Notch_pathway")
proc_dir <- file.path("results","processed","Figure_4")

dir.create(fig_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(proc_dir, recursive = TRUE, showWarnings = FALSE)

umap_cache_file <- file.path(proc_dir, "UMAP_coordinates_Notch.csv")
...
# ============================================
# 3. Merge + filter Notch pathway genes
# ============================================
...
notch_genes <- unique(subset(genes_transcripts_by_pathway,
                             pathway == "Notch")$gene_symbol)

expression_notch <- entire_dataset %>%
  filter(gene_id %in% notch_genes)

# ============================================
# 4. Prepare data for PCA
# ============================================

rownames(expression_notch) <- expression_notch$transcript_id
expression_notch_clean <- expression_notch[, !(names(expression_notch) %in% c("gene_id", "transcript_id"))]
expression_notch_t <- t(expression_notch_clean)

expression_notch_t <- expression_notch_t[order(rownames(expression_notch_t)), ]

# ============================================
# 5. PCA
# ============================================

pca_notch <- prcomp(expression_notch_t, scale. = TRUE)

metadata_ordered <- metadata[
  match(rownames(expression_notch_t), metadata$PatientID.SpecimenID.SampleID),
]

pca_df <- as.data.frame(pca_notch$x)
...
p_pca <- ggplot(pca_df, aes(PC1, PC2, color = Passage)) +
  ...
  theme_minimal() +
  labs(
    title="PCA of Notch pathway gene expression",
...
ggsave(
  filename = file.path(fig_dir, "PCA_Notch.tiff"),
  plot = p_pca,
  width = 6,
  height = 5,
  dpi = 300
...
if (!file.exists(umap_cache_file)) {
  ...
  umap_df$SampleID <- rownames(expression_notch_t)
...
p_umap <- ggplot(umap_df, aes(UMAP1,UMAP2,color=Passage)) +
  geom_point(size=3) +
  theme_minimal() +
  labs(title="UMAP of Notch pathway gene expression") +
  scale_color_manual(values=c(
    "Resection"="black","0"="orange","1"="darkgoldenrod",
    "2"="green","3"="blue","4"="purple","+5"="red"
  )) +
...
ggsave(
  filename = file.path(fig_dir, "UMAP_Notch.tiff"),
  plot = p_umap,
  width = 6,
  height = 5,
  dpi = 300
...
# ============================================
# 9.1 Estimate an appropriate number of clusters
# ============================================
...
tiff(file.path(fig_dir, "Elbow_Kmeans_Notch.tiff"),
...
tiff(file.path(fig_dir, "Silhouette_Kmeans_Notch.tiff"),
     width = 6, height = 5, units = "in", res = 300)
...
# ============================================
# 9.2 k-means clustering on UMAP
# ============================================
...
p_clusters <- ggplot(umap_df, aes(x = UMAP1, y = UMAP2, color = Cluster)) +
  geom_point(size = 3) +
  theme_minimal() +
  labs(title = "k-means clusters on Notch UMAP") +
  scale_color_manual(values = cluster_colors_cb) +
  theme(plot.title = element_text(hjust = 0.5),
        panel.grid = element_blank())

ggsave(file.path(fig_dir, "UMAP_Clusters_Notch.tiff"),
...
p_clusters_ell <- ggplot(umap_df, aes(x = UMAP1, y = UMAP2, color = Cluster)) +
  ...
  theme_minimal() +
  labs(title = "k-means clusters on Notch UMAP (with ellipses)") +
  scale_color_manual(values = cluster_colors_cb) +
  theme(plot.title = element_text(hjust = 0.5),
        panel.grid = element_blank())

ggsave(file.path(fig_dir, "UMAP_Clusters_Notch_Ellipses.tiff"),
...
ggsave(file.path(fig_dir,"Distribution_Passages_Clusters_Notch.tiff"),
...
ggsave(file.path(fig_dir, "Alluvial_Clusters_Notch.tiff"),
       p_alluvial, width = 6, height = 5, dpi = 300)

message("Figure 4C (Notch PCA/UMAP) completed.")
