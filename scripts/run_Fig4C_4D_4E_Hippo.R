# ============================================================
# run_Figure4C_Hippo_PCA_UMAP.R
#
# PCA, UMAP and clustering of Hippo pathway genes in PDMR
#
# Inputs:
...
# - results/Figures/Figure_4/Hippo_pathway/*.tiff
#
# Intermediate:
# - results/processed/Figure_4/UMAP_coordinates_Hippo.csv
# ============================================================
...
fig_dir  <- file.path("results","Figures","Figure_4","Hippo_pathway")
proc_dir <- file.path("results","processed","Figure_4")

dir.create(fig_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(proc_dir, recursive = TRUE, showWarnings = FALSE)

umap_cache_file <- file.path(proc_dir, "UMAP_coordinates_Hippo.csv")
...
# ============================================
# 3. Merge + filter Hippo pathway genes
# ============================================
...
Hippo_genes <- unique(subset(genes_transcripts_by_pathway,
                             pathway == "Hippo")$gene_symbol)

expression_Hippo <- entire_dataset %>%
  filter(gene_id %in% Hippo_genes)

# ============================================
# 4. Prepare data for PCA
# ============================================

rownames(expression_Hippo) <- expression_Hippo$transcript_id
expression_Hippo_clean <- expression_Hippo[, !(names(expression_Hippo) %in% c("gene_id", "transcript_id"))]
expression_Hippo_t <- t(expression_Hippo_clean)

expression_Hippo_t <- expression_Hippo_t[order(rownames(expression_Hippo_t)), ]

# ============================================
# 5. PCA
# ============================================

pca_Hippo <- prcomp(expression_Hippo_t, scale. = TRUE)

metadata_ordered <- metadata[
  match(rownames(expression_Hippo_t), metadata$PatientID.SpecimenID.SampleID),
]

pca_df <- as.data.frame(pca_Hippo$x)
...
p_pca <- ggplot(pca_df, aes(PC1, PC2, color = Passage)) +
  ...
  theme_minimal() +
  labs(
    title="PCA of Hippo pathway gene expression",
...
ggsave(
  filename = file.path(fig_dir, "PCA_Hippo.tiff"),
  plot = p_pca,
  width = 6,
  height = 5,
  dpi = 300
...
if (!file.exists(umap_cache_file)) {
  ...
  umap_df$SampleID <- rownames(expression_Hippo_t)
...
p_umap <- ggplot(umap_df, aes(UMAP1,UMAP2,color=Passage)) +
  geom_point(size=3) +
  theme_minimal() +
  labs(title="UMAP of Hippo pathway gene expression") +
  scale_color_manual(values=c(
    "Resection"="black","0"="orange","1"="darkgoldenrod",
    "2"="green","3"="blue","4"="purple","+5"="red"
  )) +
...
ggsave(
  filename = file.path(fig_dir, "UMAP_Hippo.tiff"),
  plot = p_umap,
  width = 6,
  height = 5,
  dpi = 300
...
tiff(file.path(fig_dir, "Elbow_Kmeans_Hippo.tiff"),
...
tiff(file.path(fig_dir, "Silhouette_Kmeans_Hippo.tiff"),
     width = 6, height = 5, units = "in", res = 300)
plot(
  2:10, silhouette_scores, type = "b", pch = 19, frame = FALSE,
  xlab = "Number of clusters K",
...
p_clusters <- ggplot(umap_df, aes(x = UMAP1, y = UMAP2, color = Cluster)) +
  geom_point(size = 3) +
  theme_minimal() +
  labs(title = "k-means clusters on Hippo UMAP") +
  scale_color_manual(values = cluster_colors_cb) +
  theme(plot.title = element_text(hjust = 0.5),
        panel.grid = element_blank())

ggsave(file.path(fig_dir, "UMAP_Clusters_Hippo.tiff"),
...
p_clusters_ell <- ggplot(umap_df, aes(x = UMAP1, y = UMAP2, color = Cluster)) +
  ...
  theme_minimal() +
  labs(title = "k-means clusters on Hippo UMAP (with ellipses)") +
  scale_color_manual(values = cluster_colors_cb) +
  theme(plot.title = element_text(hjust = 0.5),
        panel.grid = element_blank())

ggsave(file.path(fig_dir, "UMAP_Clusters_Hippo_Ellipses.tiff"),
...
ggsave(file.path(fig_dir,"Distribution_Passages_Clusters_Hippo.tiff"),
...
ggsave(file.path(fig_dir, "Alluvial_Clusters_Hippo.tiff"),
       p_alluvial, width = 6, height = 5, dpi = 300)

message("Figure 4C (Hippo PCA/UMAP) completed.")
