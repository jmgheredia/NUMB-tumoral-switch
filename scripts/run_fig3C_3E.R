# ============================================================
# Figure 3C (GO) + Figure 3E (KEGG) — NUMB isoform enrichment
# ============================================================
...
fig3C_dir  <- file.path("results","figures","figure3C")
fig3E_dir  <- file.path("results","figures","figure3E")
tables_dir <- file.path("results","Tables")

dir.create(fig3C_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(fig3E_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(tables_dir, recursive = TRUE, showWarnings = FALSE)

stopifnot(file.exists(diff_file))

out_fig_3C   <- file.path(fig3C_dir,"Figure3C_GO_chord_top40.tiff")
out_fig_3E   <- file.path(fig3E_dir,"Figure3E_KEGG_chord_top50.tiff")
out_table_GO   <- file.path(tables_dir,"NUMB_isoforms_GO_enrichment.csv")
...
message("GO enrichment table saved to: ", out_table_GO)
...
clasificar_go <- function(df) {
  
  terms <- unique(df$ID)
  grupo <- character(length(terms))
  ...
  for (i in seq_along(terms)) {
    ...
    if (ups>0 & downs==0) grupo[i] <- "up"
    else if (downs>0 & ups==0) grupo[i] <- "down"
    else grupo[i] <- "mixto"
  }
  
  data.frame(ID=terms,grupo)
}

grupos_go <- clasificar_go(go_top)

orden_go_ids <- c(
  grupos_go$ID[grupos_go$grupo=="up"],
  grupos_go$ID[grupos_go$grupo=="mixto"],
  grupos_go$ID[grupos_go$grupo=="down"]
)

go_top$ID <- factor(go_top$ID, levels=orden_go_ids)
...
go_colors <- setNames(
  colorRampPalette(brewer.pal(9,"Set1"))(length(orden_go_ids)),
  orden_go_ids
)

all_colors_go <- c(isoform_colors_go, go_colors)

tiff(out_fig_3C, width=3000, height=3000, res=600, compression="lzw")

circos.clear()
circos.par(gap.degree=2,start.degree=90)

chordDiagram(relations_go,
...
circos.trackPlotRegion(track.index=1,
                       panel.fun=function(x,y){
                         ...
                         circos.text(mean(xl),0,sn,facing="clockwise",niceFacing=TRUE,adj=c(0,0.5),cex=0.5)
                       },
                       bg.border=NA)
...
message("Figure 3C saved to: ", out_fig_3C)
...
message("KEGG enrichment table saved to: ", out_table_KEGG)
...
grupos_kegg <- clasificar_kegg(kegg_top)

orden_kegg_ids <- c(
  grupos_kegg$ID[grupos_kegg$grupo=="up"],
  grupos_kegg$ID[grupos_kegg$grupo=="mixto"],
  grupos_kegg$ID[grupos_kegg$grupo=="down"]
)

kegg_top$ID <- factor(kegg_top$ID,levels=orden_kegg_ids)
kegg_top <- kegg_top[order(kegg_top$ID),]

relations <- kegg_top[,c("Isoform","ID")]

isoform_colors <- isoform_colors_go

kegg_colors <- setNames(
  colorRampPalette(brewer.pal(9,"Set1"))(length(orden_kegg_ids)),
  orden_kegg_ids
)

all_colors <- c(isoform_colors,kegg_colors)

tiff(out_fig_3E,width=3000,height=3000,res=600,compression="lzw")
...
message("Figure 3E saved to: ", out_fig_3E)
