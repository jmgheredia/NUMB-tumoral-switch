# ============================================================
# run_Figure4C_WNT_PCA_UMAP.R
#
# PCA, UMAP and clustering of WNT pathway genes in PDMR
#
# Inputs:
...
# - results/Figures/Figure_4/WNT_pathway/*.tiff
#
# Intermediate:
# - results/processed/Figure_4/UMAP_coordinates_WNT.csv
# ============================================================
...
fig_dir  <- file.path("results","Figures","Figure_4","WNT_pathway")
proc_dir <- file.path("results","processed","Figure_4")

dir.create(fig_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(proc_dir, recursive = TRUE, showWarnings = FALSE)

umap_cache_file <- file.path(proc_dir, "UMAP_coordinates_WNT.csv")
...
# ============================================
# 3. Merge + filter WNT pathway genes
# ============================================
...
wnt_genes <- unique(subset(genes_transcripts_by_pathway,
                           pathway == "WNT")$gene_symbol)

expression_wnt <- entire_dataset %>%
  filter(gene_id %in% wnt_genes)

# ============================================
# 4. Prepare data for PCA
# ============================================

rownames(expression_wnt) <- expression_wnt$transcript_id
expression_wnt_clean <- expression_wnt[, !(names(expression_wnt) %in% c("gene_id", "transcript_id"))]
expression_wnt_t <- t(expression_wnt_clean)

expression_wnt_t <- expression_wnt_t[order(rownames(expression_wnt_t)), ]

# ============================================
# 5. PCA
# ============================================

pca_wnt <- prcomp(expression_wnt_t, scale. = TRUE)

metadata_ordered <- metadata[
  match(rownames(expression_wnt_t), metadata$PatientID.SpecimenID.SampleID),
]

pca_df <- as.data.frame(pca_wnt$x)
...
p_pca <- ggplot(pca_df, aes(PC1, PC2, color = Passage)) +
  ...
  theme_minimal() +
  labs(
    title="PCA of WNT pathway gene expression",
...
ggsave(
  filename = file.path(fig_dir, "PCA_WNT.tiff"),
  plot = p_pca,
  width = 6,
  height = 5,
  dpi = 300
...
if (!file.exists(umap_cache_file)) {
  ...
  umap_df$SampleID <- rownames(expression_wnt_t)
...
p_umap <- ggplot(umap_df, aes(UMAP1,UMAP2,color=Passage)) +
  geom_point(size=3) +
  theme_minimal() +
  labs(title="UMAP of WNT pathway gene expression") +
  scale_color_manual(values=c(
    "Resection"="black","0"="orange","1"="darkgoldenrod",
    "2"="green","3"="blue","4"="purple","+5"="red"
  )) +
...
ggsave(
  filename = file.path(fig_dir, "UMAP_WNT.tiff"),
  plot = p_umap,
  width = 6,
  height = 5,
  dpi = 300
...
tiff(file.path(fig_dir, "Elbow_Kmeans_WNT.tiff"),
...
tiff(file.path(fig_dir, "Silhouette_Kmeans_WNT.tiff"),
     width = 6, height = 5, units = "in", res = 300)
...
p_clusters <- ggplot(umap_df, aes(x = UMAP1, y = UMAP2, color = Cluster)) +
  geom_point(size = 3) +
  theme_minimal() +
  labs(title = "k-means clusters on WNT UMAP") +
  scale_color_manual(values = cluster_colors_cb) +
  theme(plot.title = element_text(hjust = 0.5),
        panel.grid = element_blank())

ggsave(file.path(fig_dir, "UMAP_Clusters_WNT.tiff"),
...
p_clusters_ell <- ggplot(umap_df, aes(x = UMAP1, y = UMAP2, color = Cluster)) +
  ...
  theme_minimal() +
  labs(title = "k-means clusters on WNT UMAP (with ellipses)") +
  scale_color_manual(values = cluster_colors_cb) +
  theme(plot.title = element_text(hjust = 0.5),
        panel.grid = element_blank())

ggsave(file.path(fig_dir, "UMAP_Clusters_WNT_Ellipses.tiff"),
...
ggsave(file.path(fig_dir,"Distribution_Passages_Clusters_WNT.tiff"),
...
ggsave(file.path(fig_dir, "Alluvial_Clusters_WNT.tiff"),
       p_alluvial, width = 6, height = 5, dpi = 300)

message("Figure 4C (WNT PCA/UMAP) completed.")
